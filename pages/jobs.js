...
const Jobs = () => {
  ...
  const [detailRender, setDetailRender] = useState({display:false, render: <JobCardDetailsMobile /> })
  ...
  const handleClick = (e) => {
    ...
    setDetailRender({display:true, render: <JobCardDetailsMobile {...selectedCard} onClick={() => setDetailRender({display:false, render: <JobCardDetailsMobile /> }) }/>});
  }

  switch (jobs.state) {
    case "hasValue":
      
      const postingList = jobs.contents.data.data.postings;
